...
const fallbackProjects: ProjectListItem[] = [
  {
    ...
    subtitle: "可追问式简历体验",
    summary:
      "基于 Dify API 的可追问式简历体验，重点体现对话 UI、SSE 流式渲染、后端代理和项目证据组织。",
    project_type: "AI 应用",
  ...
  },
  {
    ...
    subtitle: "作品集内容 API",
    summary:
      "FastAPI + SQLAlchemy 管理项目、媒体、简历和预设问题，并通过 Bearer token 保护后台接口。",
    project_type: "后端",
    role: "接口设计、数据建模、测试覆盖",
  ...
  },
  {
    ...
    summary:
      "手机端优先保留 AI 对话入口，让招聘方在微信或浏览器中打开时，也能直接提问并查看关键材料。",
    project_type: "前端",
        ...
        {visibleProjects.map((project) => (
          <article className="project-card" key={project.id}>
            ...
            <div className="project-content">
              ...
              {project.subtitle ? (
                <strong className="project-subtitle">{project.subtitle}</strong>
              ) : null}
              <p>{project.summary}</p>
              <div className="stack">
              ...
              <a className="detail-link" href={projectDetailPath(project.id)}>
                查看项目详情 →
              </a>
            </div>
          </article>
